...
function LineView(props) {
    return (
        <React.Fragment>
            {withUniqueKeys(props.line.getNormalizedHunks(props.statusComingFirst)).map(hunk => (
    ...
    return (
        <p>
            {withUniqueKeys(props.paragraph.lines).map(line => (
                <LineView line={line} statusComingFirst={props.statusComingFirst} key={line.key} />
            ))}
        </p>
    );
}
...
function PageDbg(props) {
    return (
        <div>
            ...
            {Object.keys(props.page.content).length} - {props.page.start} to {props.page.end}
        </div>
    );
}
...
export default class HistoryView extends React.Component {
    ...
    componentDidMount() {
        ...
        Promise.all([this.props.history.ensureIndex(this.props.startAt), later(2000)]).then(() => {
            this.setState({
                loading: false,
                starting: true,
    ...
    renderText() {
        ...
        return (
            <div className="container">
                ...
                <DiffViewer from={from} to={to} goingForward={this.state.goingForward} />
